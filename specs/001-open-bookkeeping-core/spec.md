...
- **FR-004**: 系統 MUST 在儲存紀錄前驗證資料完整性（金額為正數、日期為有效格式、分類已選擇）
...
- **FR-021**: 系統 MUST 允許使用者將收支紀錄匯出為 CSV 格式（欄位：日期,類型,金額,分類,帳戶,備註）
- **FR-022**: 系統 MUST 支援匯出時選擇日期範圍篩選
- **FR-023**: 系統 MUST 正確處理 CSV 中的特殊字元（逗號、引號、換行）
- **FR-024**: 系統 MUST 允許使用者匯入標準格式 CSV（V1 固定欄位順序：日期,類型,金額,分類,帳戶,備註）
...
- **Transaction（交易紀錄）**: 每一筆收入或支出的記錄。關鍵屬性：唯一識別碼、日期、金額（正數）、類型（收入/支出）、分類、帳戶、備註、標籤、建立時間、更新時間。與 Category 和 Account 有關聯。
...
- 預算週期以月為最小單位，週預算為加分項
- 應用部署目標為靜態網頁伺服器（如 GitHub Pages），不依賴後端服務
